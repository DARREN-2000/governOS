  ...
  try {
    ...
    if (description.toLowerCase().includes("github")) {
      ...
      const res = await github.execute(
        { action: "create_issue", owner, repo, title: description },
        planId,
      );
      console.log("Github Action Result:", res);
    } else if (
      description.toLowerCase().includes("ec2") ||
      ...
      console.log(
        "No specific action matched description, running generic completion.",
      );
    }
    ...
    await run(
      "INSERT INTO audit_events (workflow_id, event_type) VALUES (?, ?)",
      [planId, "PLAN_EXECUTED"],
    );
  } catch (e) {
    console.error(e);
