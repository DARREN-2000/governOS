...
export async function runFallbackWorker() {
  ...
  while (true) {
    try {
      const workflow: any = await get(
        "SELECT * FROM workflows WHERE status = 'approved' LIMIT 1",
      );
      if (workflow) {
        console.log(`Executing fallback workflow ${workflow.id}`);
        await run("UPDATE workflows SET status = 'executing' WHERE id = ?", [
          workflow.id,
        ]);
        const intent: any = await get(
          "SELECT description FROM intents WHERE id = ?",
          [workflow.intent_id],
        );
        if (intent) {
