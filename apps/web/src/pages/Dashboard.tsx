...
function deriveRiskLevel(steps: { action: string; description: string }[]): string {
  const allText = steps.map((s) => `${s.action} ${s.description}`).join(" ").toLowerCase();
  if (/delete|destroy|drop|terminate|remove|wipe/.test(allText)) return "Critical";
  if (/write|create|provision|deploy|execute|update|modify/.test(allText)) return "High";
  if (/api_call|fetch|send|post|push/.test(allText)) return "Medium";
  return "Low";
}
...
export function Dashboard() {
    ...
    if (import.meta.env.VITE_API_URL) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/v1/intents`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          const normalizedSteps = (data.plan.steps || []).map(
            (step: any, idx: number) => ({
      ...
      setTimeout(() => {
        ...
        if (lowerIntent.includes("s3") || lowerIntent.includes("bucket")) {
          ...
          lowerIntent.includes("ec2") ||
          lowerIntent.includes("instance")
        ) {
        ...
        const mockPlan = {
          ...
          steps: [
            {
              id: 1,
              description: `Analyze natural language intent: "${intent}"`,
              icon: Terminal,
            },
            {
              id: 2,
              description: `Compile execution graph for action: ${action}`,
              icon: Activity,
            },
            {
              id: 3,
              description: `Target evaluation: ${target || "default"}`,
              icon: CheckCircle2,
            },
            {
              id: 4,
              description: "Dry run execution plan & policy check",
              icon: ShieldCheck,
            },
          ],
        };
  ...
  const handleApprove = async () => {
    ...
    if (import.meta.env.VITE_API_URL && workflow.db_id) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/v1/intents/${workflow.db_id}/approve`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            ...
            <div className="relative z-10">
              ...
              <div className="mt-6 flex justify-end">
                ...
                >
                  ...
                  <span>
                    {loading ? "Compiling Graph..." : "Generate Plan"}
                  </span>
                </button>
              </div>
            </div>
            ...
            {workflow && (
              ...
              >
                <div className="flex justify-between items-start mb-8">
                  <div>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-slate-400 font-mono">
                      <span className="flex items-center">
                        <Fingerprint size={14} className="mr-1" /> {workflow.id}
                      </span>
                      <span className="flex items-center">
                        <Lock size={14} className="mr-1" /> Risk:{" "}
                        <span
                          className={`ml-1 font-bold ${workflow.riskLevel === "Critical" ? "text-rose-400" : workflow.riskLevel === "High" ? "text-orange-400" : "text-emerald-400"}`}
                        >
                          {workflow.riskLevel}
                        </span>
                      </span>
                  ...
                  <div
                    className={`px-4 py-2 rounded-xl text-sm font-bold border backdrop-blur-md flex items-center shadow-lg
                    ${
                  ...
                  {workflow.steps.map((step: any, idx: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="font-mono text-[15px] text-slate-200">
                          {step.description}
                        </span>
                      </motion.div>
                    );
                ...
                {workflow.status !== "completed" && (
                  ...
                  >
                    ...
                    <p className="text-center text-xs text-slate-500 mt-4">
                      By approving, you authorize governOS to perform these
                      actions via the execution agent.
                    </p>
                  </motion.div>
                )}
              </motion.section>
