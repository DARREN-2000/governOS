...
export function Playground() {
  ...
  const [output, setOutput] = useState<
    {
      type: "command" | "plan" | "success" | "error" | "info";
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    setOutput((prev) => [
      ...prev,
      { type: "command", text: `➜ governos plan "${newCommand}"` },
    ]);
          ...
          text: (
            <div className="mt-2 text-white/80">
              ...
              <ul className="list-disc pl-5 mt-1 space-y-1 grid grid-cols-1 sm:grid-cols-2 gap-x-4">
                <li>
                  <span className="text-primary">deploy [app]</span> - Simulate
                  a deployment
                </li>
                <li>
                  <span className="text-primary">scale [service] [number]</span>{" "}
                  - Simulate scaling a service
                </li>
                <li>
                  <span className="text-primary">restart [pod]</span> - Simulate
                  restarting a resource
                </li>
                <li>
                  <span className="text-primary">rollback [app]</span> -
                  Rollback a deployment
                </li>
                <li>
                  <span className="text-primary">provision [resource]</span> -
                  Provision infrastructure
                </li>
                <li>
                  <span className="text-primary">destroy [resource]</span> -
                  Destroy resources
                </li>
                <li>
                  <span className="text-primary">analyze [target]</span> -
                  Analyze system state
                </li>
                <li>
                  <span className="text-primary">audit [target]</span> - Run
                  security audit
                </li>
                <li>
                  <span className="text-primary">status</span> - View system
                  status
                </li>
                <li>
                  <span className="text-primary">history</span> - View execution
                  history
                </li>
                <li>
                  <span className="text-primary">whoami</span> - Display current
                  identity
                </li>
                <li>
        ...
        { type: "success", text: "✔ Policy checks passed" },
        {
          ...
          text: (
            <div className="mt-4 mb-4 p-4 border border-white/10 bg-black/40 rounded-lg">
              <p className="text-white/90 font-bold border-b border-white/10 pb-2 mb-2">
                  ...
                  {risk === "Critical"
                    ...
                    : risk === "High"
                      ...
                      : risk === "Medium"
                        ? "⚠️"
                        : "✅"}{" "}
                  Risk Level: {risk} {risk === "Low" && "(Auto-approved)"}
                </p>
                <span className="text-xs text-muted-foreground">
                  Est. Time: {(delay / 1000).toFixed(1)}s
                </span>
              </div>
            </div>
          ),
    ...
    <div className="container px-4 py-32 mx-auto md:px-6 min-h-screen flex flex-col items-center">
      ...
      >
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          Try out GovernOS locally in this mocked environment. Type an intent to
          see how it plans execution.
        </p>
      </motion.div>
            ...
            <div className="flex-1 space-y-1">
              ...
              <div className="mt-4 mb-4 flex flex-wrap gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs bg-white/5 border-white/10 text-muted-foreground hover:text-white"
                ...
                <Button
                  ...
                  size="sm"
                  className="h-6 text-xs bg-white/5 border-white/10 text-muted-foreground hover:text-white"
                  onClick={() => setCommand("analyze frontend")}
                >
                  analyze frontend
                </Button>
                <Button
                ...
                <Button
                  ...
                  size="sm"
                  className="h-6 text-xs bg-white/5 border-white/10 text-muted-foreground hover:text-white"
                  onClick={() => setCommand("provision database")}
                >
                  provision database
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs bg-white/5 border-white/10 text-muted-foreground hover:text-destructive"
                  onClick={() => setCommand("destroy cache-cluster")}
                >
                  destroy cache-cluster
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                <div key={i}>
                  ...
                  {item.type === "error" && (
                    <p className="text-destructive flex items-center gap-2">
                      <span className="text-destructive">✖</span> {item.text}
                    </p>
                  )}
